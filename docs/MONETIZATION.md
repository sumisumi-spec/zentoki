...
- **名称**: ZENTOKI Plus。**買い切り 500円**（税込）。サブスクなし、自動更新なし。
...
1. **Airペイ 管理画面**（https://merchant.online.airpayment.jp/plan/list）
   1. 「都度決済」のプランを作成: 商品名 `ZENTOKI Plus（買い切り）`、金額 500 円。
...
1. **Stripe**（https://dashboard.stripe.com）
   1. 商品を作成: 名前 `ZENTOKI Plus`、価格 500 円、一回払い。
...
1. **App Store Connect** → ZENTOKI → App 内課金 → 「+」
   - 種類: **非消耗型** / 参照名 `ZENTOKI Plus` / 製品 ID `app.zentoki.plus` / 価格 500 円（Tier）
...
- Web（Airペイ オンライン決済）: 手数料 3.24%、振込手数料 0 円、入金は月 1 回 → 500 円あたり約 484 円。
- Web（Stripe）: 手数料 3.6% → 500 円あたり約 482 円。
- iOS: App Store 手数料 15%（小規模事業者プログラム申請後） → 約 425 円。
- 100 人が購入で 約 4.3〜4.8 万円。運用コストは Worker 0 円、Pages 0 円。
